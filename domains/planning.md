...
queue. Declared by the **convergent planning/decomposition** composition. Provenance and per-kill
detail in `domains/audit.md`.
...
- id: sequence-by-output-dependency
  ...
  reason: "Composition order (UX-flow before visual-identity before implementation) is a heuristic, not a law. It breaks when tasks within a capability don't align with that order. Output dependency is the correct sequencing signal — it holds regardless of who does the work."

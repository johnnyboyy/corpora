...
- id: mined-workflow-stays-a-workflow
  rule: "When a source documents a coherent, ordered workflow rather than independent decision points, don't atomize each step into a standalone principle. Extract only the mechanism-level gotchas that fire independent of the workflow's sequencing; leave the sequencing itself unencoded rather than force it into disconnected `rule`/`condition`/`reason` atoms."

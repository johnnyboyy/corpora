...
Judgment about security-relevant decisions made before there's a diff to scan — scope, trust
boundaries, defaults, and dependency choices under ambiguity — distinct from `coding-general`'s
general implementation judgment and from automated pattern/LLM-diff security scanning, which
catches shipped-code patterns (injection, XSS, hardcoded secrets, unsafe deserialization) after
the fact. This domain holds what that reactive layer structurally can't catch: decisions that
depend on intent, scope, or architecture rather than a pattern in the code itself. Seeded
2026-08-02 from an independent agent's proposal, reviewed and ratified against the genuine-fork
test and the agent-vs-human consuming-lens check. Audit metadata lives in `domains/audit.md`,

...
- id: effect-only-derived-state-belongs-in-render
  ...
  reason: "The effect only defers a derivable computation to a second render pass for no benefit — an extra render plus an unneeded node in the effect dependency graph. An effect whose entire body has zero external interaction is, by construction, doing work the render body could do directly; deferring it into an effect only adds indirection and a stale-render window with nothing to show for it."

...
- id: route-questions-not-roles
  rule: "Route a UX or UI question to its owning designer, not to the operator by default. If non-blocking, queue it to the owning role's queue (`corpora/deferred-decisions.md`) for resolution at that role's next natural spawn — including questions raised by a coder session, not only the designer's own self-deferrals. If blocking and the role's judgment is needed now, spawn or resume that role; a live role that hits a genuine question mid-work can pause (`questions-pending`) and resume once answered. Surface to the operator directly only when neither path fits."

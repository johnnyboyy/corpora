...
and from `planning`'s judgment about decomposing a capability once ambiguity is resolved. Stance-
agnostic: consumed by any convergent lens that hits a genuine clarifying moment, not owned by a
single role. Seeded 2026-07-22 from decomposing `planner.md`'s "Dialogue" step and absorbing
`orchestrator-routing`'s `surface-design-questions-neutrally` (see that principle's audit history).
...
- id: frame-questions-for-cheap-answers
  ...
  condition: "When asking a clarifying or direction question of the operator or of another role, whether during a planner's dialogue, an orchestrator routing a design question outward, or any lens's `questions-pending` pause."

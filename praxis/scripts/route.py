...
*execution-shape signals* on top —
  ...
  - **resume vs new**: if a workstream id is named, a ledger lookup says whether prior chunks for it
    already exist (a *resume* candidate) or not (*new*). The lookup is the read-only `close-workstream`
    engine capability, resolved generically through the manifest — praxis never names a corpora verb.
...
def workstream_ledger_state(corpus_py: Path, workstream: str | None) -> tuple[str, str]:
    """Read-only resume-vs-new signal. Returns (state, note) where state is one of:
    'exists' (a ledger for this workstream is present → resume candidate),
    'absent' (engine answered, no such ledger → new),
    'unknown' (no workstream named, or the engine was unavailable → cannot tell, defaults to new).

    The one engine touch is the `close-workstream` capability (a read-only ledger summary), resolved
    by NAME through the manifest — no corpora verb is named here, exactly as `frame` names `compose`.
    """
    if not workstream:
        return "unknown", "no workstream named — treated as new work"
    res = engine.resolve(Path(corpus_py), "close-workstream", {"workstream": workstream})
    if not res.ran:
        return "unknown", res.note()
    if res.returncode == 0:
        return "exists", "a chunk ledger for this workstream exists — resume candidate"
    return "absent", "engine reports no ledger for this workstream — new work"
...
def build_route(base: Path, target: str | None, files: list[str], unit_of_work: str | None,
    ...
    ledger_state, ledger_note = workstream_ledger_state(corpus_py, workstream)

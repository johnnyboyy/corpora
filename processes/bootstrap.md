...
## project-shape
name: <optional — a label for this corpora root, for corpus.py --root-name/list-roots
       (kernel.md, "Monorepo root resolution"); defaults to the root directory's own
       basename when omitted, so only set this when that default isn't a good name>
